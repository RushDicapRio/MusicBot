class Util {
    constructor() {
        ...
        this.parseDur = function parseDur(ms) {
            ...
            if (days) {
                return `${days} jour, ${hours} heures, ${minutes} minutes`;
            } else if (hours) {
        ...
        this.getUserBannerURL = async function (client, userId, { dynamicFormat = true, defaultFormat = "webp", size = 512 } = {}) {
            ...
            if (![16, 32, 64, 128, 256, 512, 1024, 2048, 4096].includes(size)) {
                throw new Error(`La taille '${size}' n'est pas pris en charge !`);
            }
            ...
            if (!["webp", "png", "jpg", "jpeg"].includes(defaultFormat)) {
                throw new Error(`Le format '${defaultFormat}' n'est pas pris en charge comme un format par défaut !`);
            }
