...
module.exports = async (client, message) => {
    ...
    if (!timestamps.has(member.id)) {
        ...
        if (now < expirationTime) {
            ...
            return message.channel.send(`Calme-moi , attends **${timeLeft.toFixed(1)}** De nouvelles secondes peuvent utiliser.`);
    ...
    try {
        ...
        console.log(`${sender.tag} (${sender.id}) courue ${cmd}`); // Mengetahui, siapa aja yang make command (cuman di console log aja kok)
    }
};